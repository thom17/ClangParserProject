...
def edit_unit(editor: CodeEditor):
    ...
    editor.add_define('#include "../DataAccessor/DAutoGenTester.h"', check_dup=False)
    editor.write_file()
...
def get_planned_code(method_cursor: Cursor, head_unit: CUnit) -> str:
    ...
    if '.' in src_name: #method_cursor == 'CXX_METHOD': (생성자, 소멸자 등 단순 처리로는 복잡할듯. 뭔가 host 개념이 있다면 . 이 있을것
        ...
        if not is_static:
            return f'DAutoGenDB debugGen(CString(L"{src_name}::") + CString(typeid(*this).name()));'

    return f'DAutoGenDB debugGen(L"{src_name}");'
